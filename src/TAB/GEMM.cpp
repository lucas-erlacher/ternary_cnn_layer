...
std::vector<int> TNNGEMM_baseline(int64_t* a, int64_t* b, int64_t M, int64_t N, int64_t K) {
    std::vector<int> y = std::vector<int>(M * N, 0);
    const int64_t KB = K * BITS;
    for (int64_t oh = 0; oh < M; oh++) {
        for (int64_t ow = 0; ow < N; ow++) {
            int cntp1 = 0;
            int cntp2 = 0;
            for (int64_t ik = 0; ik < KB; ik += BITS) {
                // Use H_W_B format
